...
function validateChecksum(letters, checksum) {
...
function findRealRooms() {
  let sum = 0;
  const potentialRooms = input.split('\n')
    .map(x =>
  ...
  for(let i = 0; i < potentialRooms.length; ++i) {
    ...
    for (let j = 0; j < room[0].length; j++) {
      if(letterMap[room[0][j]]) {
        ++letterMap[room[0][j]];
      } else letterMap[room[0][j]] = 1;
    }
    let letterArr = [];
    _.forOwn(letterMap, (value, key) => {
      letterArr
        .push([key, value])
    });
    letterArr.sort((x, y) => {
    ...
    sum += validateChecksum(letterArr, room[2]) ? parseInt(room[1]) : 0;
    console.log(sum);
  }
}
