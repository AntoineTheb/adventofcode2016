...
function findABBA() {
  ...
  for (let i = 0; i < adresses.length; ++i) {
    ...
    for (let j = 0; j < adresses[i].length; ++j) {
      if (j % 2 === 0) aba.push(adresses[i][j]);
      else bab.push(adresses[i][j]);
    }

    const straba = aba.join();
    const strbab = bab.join();
    for (let j = 0; j < straba.length - 2; j++) {
      if (straba[j] === straba[j+2] &&
        straba[j] !== straba[j+1]) {
          if(new RegExp(`${straba[j+1]}${straba[j+2]}${straba[j+1]}`, 'g').exec(strbab)) {
            console.log(strbab,`${straba[j+1]}${straba[j]}${straba[j+1]}`, `${straba[j]}${straba[j+1]}${straba[j+2]}`);
            valids = valids + 1;
            break;
          }
      }
