...
function validateSubstring(sub) {
  for (let i = 0; i < sub.length - 3; ++i) {
    if (sub[i] !== sub[i+1] &&
      sub[i+3] === sub[i] &&
      sub[i+2] === sub[i+1]) {
        return `${sub[i]}${sub[i+1]}${sub[i+2]}${sub[i+3]}`;
    }
  }
  return false;
}
...
function findABBA() {
  const adresses = input.split('\n')
    .map(address => address.split(/\]|\[/));
  let valids = 0;
  for (let i = 0; i < adresses.length; ++i) {
    let isValidOutsideBrackets = false;
    let isValidInsideBrackets = false;
    for(let j = 0; j < adresses[i].length; ++j) {
      if (j % 2 === 0) {
        isValidOutsideBrackets = isValidOutsideBrackets || !!validateSubstring(adresses[i][j]);
      } else {
        isValidInsideBrackets = !!!validateSubstring(adresses[i][j]);
        if (!isValidInsideBrackets) break;
      }
    }
    if (isValidOutsideBrackets && isValidInsideBrackets) {
      console.log(adresses[i].join('|'), 'is valid');
      ++valids;
    }
  }
  console.log(valids);
